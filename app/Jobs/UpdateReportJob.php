...
class UpdateReportJob implements ShouldQueue
{
    ...
    {
        // $listereners = UserConference::with('user')->where('conference_id', $this->conference_id)->get();
        // foreach($listereners as $listerener) {
        //     if($listerener->user->role === 'Listener') {
        //         Mail::to($listerener->user->email)->send(new UpdateReport($this->conference_title, $this->username, $this->topic, $this->report_start, $this->report_end));
        //     }
        // }

        Mail::to('prudnyk_da@groupbwt.com')->send(new UpdateReport($this->conference_title, $this->username, $this->topic, $this->report_start, $this->report_end));
    }
}
